...
=======
    ...
    jQuery.validator.addMethod("check-password", function(value, element, breaches) {
        return breaches <= breachThreshold;
    }, jQuery.validator.format("The password security policy does not allow use of passwords that have been exposed by a past data breach. Please enter a different password."));

    if (breachThreshold != -1) {
        //user is "finished typing," run the check against exposed password list.
        function doneTyping() {
            var ajaxData = {};
            ajaxData["password"] = $(passwordInput).val();
            ajaxData[site.csrf.keys.name] = site.csrf.name;
...
>>>>>>> 5c343868... Change return types in PasswordSecurity and other minor changes
        }
      });
    }
  }
