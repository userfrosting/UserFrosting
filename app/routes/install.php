<?php
    ...
    /**
     * Routes for /install/* URLs.  Handles activities for installing UserFrosting.  Not needed after installation is complete.
     *
     */
    
    use \UserFrosting as UF;
    use \Illuminate\UF\Database\Capsule\Manager as Capsule;
     ...
     * 5. If all of these checks are passed, set up the initial tables by calling `UF\Database::install()`.
    ...
    $app->get('/install/?', function () use ($app) {
       // $controller = new UF\InstallController($app); // removeme. InstallController is not needed.
        if (isset($app->site->install_status)){
            ...
            $tables = UF\Database::getCreatedTables();
            if (count($tables) > 0){
                $messages[] = [
            ...
            if (count($messages) > 0){
                $app->render('install/install-errors.twig', [
                    "messages" => $messages
                ]);
            } else {
            error_log("Installing");
                ...
                $messages[] = [
                    "title" => "<i class='fa fa-database'></i> UF\Database connection",
                ...
                $app->render('install/install-ready.twig', [
                    "messages" => $messages
                ]);
            }
        }
    ...
    $app->get('/install/master/?', function () use ($app) {
        if (isset($app->site->install_status) && ($app->site->install_status == "pending")){
            ...
            $app->render('install/install-master.twig', [
                ...
                'table_config' => UF\Database::getSchemaTable('configuration')->name
            ]);
        } else {
    ...
    $app->post('/install/master(/:action)/?', function ($action=false) use ($app) {
        ...
        if ($post['root_account_config_token'] != $app->site->root_account_config_token) {
        ...
        $primaryGroup = UF\Group::where('is_default', GROUP_DEFAULT_PRIMARY)->first();
        $data['primary_group_id'] = $primaryGroup->id;
        // Set default title for new users
        $data['title'] = $primaryGroup->new_user_title;
        // Hash password
        $data['password'] = UF\Authentication::hashPassword($data['password']);

        // Create the master user
        $user = new UF\User($data);
        $user->id = $app->config('user_id_master');
        ...
        $app->site->install_status = "new";
        $app->site->root_account_config_token = "";
        $app->site->store();
    });
